...
int main(){
    ...
    //第一層會先做第一次 是否為0的判斷 主要是針對接收新的數字 新題目直到接收到 00
        /**
        狀況 A：輸入 0 3 或 4 0
        cin 成功讀到資料。判斷 (0 || 3) --> 0 是 false，3 是 true，
        因為是 ||（或者），只要有一個是 true 就是 true，所以結果是 true。
        
        狀況 B：輸入 0 0
        cin 成功讀到資料。判斷 (0 || 0) --> 兩個都是 false，(false || false) 結果是 false。
        因為後面變成 false，整個 while 條件就不成立了，直接跳出迴圈，結束程式。
        **/
